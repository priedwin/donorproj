...
let getbag=require('../fabcar/javascript/query_donorbankbag')
...
exports.saveDonorBloodBagDetails = async(bbid) => {
    var id = bbid.split("-");
...
exports.saveDonationDetails = async(req,res,next) => {
   ...
   this.getDonorBankBagDetails(bbid);
...
exports.getDonorBankBagListDetails = async(req,res,next) => {
    var donorbank=req.params.donor_bank
    let result1 =await getbag.getDonorBankBagDetails(donorbank);
    let transactionstring1 = JSON.parse(result1.toString());
    let t1=transactionstring1.bloodbags;
    let list1=t1.split(",");
    this.getDonorAllDonationDetails(list1,donorbank);
...
exports.getDonorAllDonationDetails = async(list1,donorbank) => {
    console.log(typeof list1);
    for(i=0;i<list1.length;i++){
        let bbid=donorbank+"-"+list1[i];
        let result1 =await getdonor.getDonationDetails(bbid);
    }
}
exports.getDonorBankBagDetails = async(bbid) => {
    var id = bbid.split("-");
    var donorbank=id[0]+"-"+id[1];
    let result1 =await getbag.getDonorBankBagDetails(donorbank);

    if(result1.toString()=="false")
    {
        this.saveDonorBloodBagDetails(bbid);
    }
    else
    {
    let transactionstring1 = JSON.parse(result1.toString());
